...
function nodeopen(id,x,y) {
  ...
  setTimeout(function () { popupnodeWindow.close();
     if(check){
  	   ...
  	   window
			.open(
					'NodeForm.jsp?floor_num='+floor_num,
					"_blank",
					"directories=no,titlebar=no,toolbar=no,location=no,status=no,menubar=no,scrollbars=yes,resizable=no,width=400,height=700,left=440,top=0");
...
<script type="text/javascript">
	...
	function changeFloor1(value, floor) {
				alert(value);
				alert(floor.id);
				plan.style.backgroundImage = 'url('+value+'_'+floor.id+'_Floor_plan.png)';
    }		
	function changeFloor(value, floor) {
		
		plan.style.backgroundImage = 'url('+value+'_'+floor+'_Floor_plan.png)';
...
<body onFocus="parent_disable();" onclick="parent_disable();" onload="changeFloor('<%=request.getParameter("building_name")%>', '<%=0%>');">
<%System.out.print("building_name "+request.getParameter("building_name")) ;
String floor_num=request.getParameter("floor") ;
%>
<div id="top" style="text-align:center;
    padding:5px; -moz-box-sizing: border-box;      
     -webkit-box-sizing: border-box; 
     box-sizing: border-box; ">
<i><a href="building_created.jsp" target="content" >Home</a></i> > 
     <i><a href="view_building_data.jsp" target="content" >Building_details</a></i>>
     <i>Upload Floor Layout</i>
</div>     
...
<form action="RegisterBuilding2" onSubmit="WriteToFile(this)" method="post" enctype="multipart/form-data">
                        ...
                        <tr><td></td>
                            <td>
                                <table>
                                    <tr>    
                                        <td colspan="4">Choose floor number and upload image</td>
                                        </tr>
                                        <tr>
                                        <td >
                                        <select name="flr_num" id="flr_num">
                                        <% 
                                        String floor = "SELECT floors FROM buildings where building_name = '"+building_name+"'";
			                            rs = s.executeQuery(floor);
			                            int no_of_floors=0;
			                            while(rs.next()){
			                            	no_of_floors=Integer.parseInt(rs.getString("floors"));
			                            }
                                        ...
                                        <td colspan="2"><input type="file" name="file1" ><!-- Here You Can Add more numbers of Files For Uploading --></td>                
                                    
                                        <td>
                                            <input type="submit" value="Upload">
<!--                                             <a href="CreateGraph.jsp"></a>
 -->                                        </td>
										<td>	
											<input type="button" id="upload-input" value="Get graph"> 
<!-- <input type="image"
        ...
        <div id="plan" style=" float:left; width:50%; height:50%;
    padding:5px; -moz-box-sizing: border-box;      
     -webkit-box-sizing: border-box; 
     box-sizing: border-box; ">
        <% if(floor_num!=null) %>
        Floor <%out.println(floor_num); %>
		...
		<%
			...
			if (connection != null) {
				...
				rs = null;
				ResultSet ry = null;
				String q1 = "Select * from node where building_name = '"+building_name+"'";/* session.getAttribute("building_name") */

				// add floor number also in the query

				rs = (s.executeQuery(q1));
				String q2 = "Select * from edge where building_name = '"+building_name+"'";/* session.getAttribute("building_name") */
