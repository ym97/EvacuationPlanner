...
<!--<input type="hidden" name="Edge_number" value = <% out.println(edge_id);%> > -->  
                                ...
                                <tr> <td>Start Node</td>
                                <td><!-- <input type="text" name="From_Node"/> -->
                                <%try {
                            	...
                            	<td>End Node</td> 
                                <td><!-- <input type="text" name="To_Node"/> -->
                                <select name="To_Node" id="To_Node">
                                    	<%
                                        
                                    	q3 = "CREATE TABLE IF NOT EXISTS node(node_id varchar(225) NOT NULL, node_name varchar(225) NOT NULL, building_name varchar(225) NOT NULL references buildings(building_name) ON DELETE CASCADE, capacity integer NOT NULL, floor integer NOT NULL, latitude float NOT NULL, longitude float NOT NULL, node_ladder varchar(5) NOT NULL default 'no', Primary key(node_id, building_name))";
                                        s.executeUpdate(q3);
                                    q1 = "Select node_id from node where building_name = '"+building_name+"'";
                                    rs = (s.executeQuery(q1));
                                    while (rs.next()) {%>
                         			   <option value=<%out.println(rs.getString("node_id"));%>>
                           		    	  <%out.println(rs.getString("node_id"));%>
